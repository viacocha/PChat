...
[![Go Report Card](https://goreportcard.com/badge/github.com/viacocha/PChat)](https://goreportcard.com/report/github.com/viacocha/PChat)
...
PChat/
├── cmd/                 # 主程序入口
│   ├── pchat/           # P2P聊天客户端（包含所有核心功能）
│   │   ├── main.go      # 主程序入口
│   │   ├── ui.go        # UI界面（可选）
│   │   ├── dht_discovery.go  # DHT发现功能
│   │   ├── registry.go  # 注册服务器客户端
│   │   └── ...          # 其他辅助文件
│   └── registry/        # 注册服务器
│       └── main.go      # 注册服务器主程序
├── internal/            # 内部包
...
# 使用DHT去中心化模式（UI界面）
./bin/pchat -port 9001 -username Alice -ui true
```
...
| 命令 | 描述 |
|------|------|
| `/sendfile <文件路径>` 或 `/file <文件路径>` 或 `/s <文件路径>` 或 `/f <文件路径>` | 发送文件给所有连接的用户 |
...
-username <用户名>  # 用户名（可选，不提供时会提示输入）
-peer <地址>        # 要连接的peer地址（格式：/ip4/127.0.0.1/tcp/端口/p2p/peerID）
-ui <true/false>    # 是否使用UI界面（默认：false）
```
...
- 🌐 个人网站: [https://tanghg.cpolar.cn](https://tanghg.cpolar.cn)
- 📧 邮箱: [tanghg.cn@gmail.com](mailto:tanghg.cn@gmail.com)
