...
**方式 1：通过命令行参数（推荐）**
```bash
./pchat -port 9001 -registry 127.0.0.1:8888 -username Alice
```

**方式 2：交互式输入**
```bash
./pchat -port 9001 -registry 127.0.0.1:8888
```
程序会提示：
```
请输入用户名（直接回车使用默认名称）: 
...
```bash
./pchat -port 9002 -registry 127.0.0.1:8888 -username Bob
```
...
- `/list` 或 `/users` - 查询在线用户列表（DHT模式或注册服务器模式）
- `/call <用户名或节点ID>` 或 `call <用户名或节点ID>` - 呼叫并连接用户
- `/sendfile <文件路径>` 或 `/file <文件路径>` - 发送文件
- `/quit` 或 `/exit` - 退出程序
